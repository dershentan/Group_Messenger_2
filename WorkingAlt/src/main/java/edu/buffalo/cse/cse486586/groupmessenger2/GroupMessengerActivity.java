...
public class GroupMessengerActivity extends Activity {
    ...
    private class ServerTask extends AsyncTask<ServerSocket, String, Void> {
                ...
                try {
                    ...
                    if(inStr != null) {
                        ...
                        if (st[0].trim().equals("Msg")) {
                            proposalCounter += 1;
                            String tempProposalCounter = Integer.toString(proposalCounter);
                            outStr = "Pps" + "\t" + whatsMyPort + "\t" + st[1].trim() + "\t" + tempProposalCounter;
                            publishProgress(inStr, tempProposalCounter);
        ...
        protected void onProgressUpdate(String...strings) {
            ...
            if (st[0].trim().equals("Msg")) {
                String currentProposalCounter = strings[1].trim();
                ...
                if (totalOrderMsgQueue.containsKey(msgFrom + messageID) && proposedValue.equals("0")) {
                    ...
                    try {
                        while (deliverable) {
                            ...
                            Log.v("Server-Agr-tomqks", totalOrderMsgQueue.keySet().toString());
    ...
    private class ClientTask extends AsyncTask<String, String, Void> {
        ...
        protected void onProgressUpdate(String...strings) {
            String[] st = strings[0].split("\\t");
            if (st[0].trim().equals("Pps")) {
                String ppsFrom = st[1].trim();
                String messageID = st[2].trim();
                String proposedValue = st[3].trim();
                Log.v("Client-Pps-Inc", messageID + " => " + ppsFrom + " : " + proposedValue);
                if (proposedValue.equals("0")) {
                    Log.v("Client-Pps-Failed", strings[0]);
                }
                ...
                if ((proposalReceived.containsKey(messageID) && proposalReceived.get(messageID).size() == 5)) {
                    ...
                    Log.v("Client-OPU-Agr", messageID + "\t" + Integer.toString(proposedPort) + "\t" + Integer.toString(maxProposedValueInMap));
